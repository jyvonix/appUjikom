<x-admin-layout>
    <div class="mb-12">
        ...
        <h1 class="text-5xl font-black text-slate-900 tracking-tight leading-tight">Perbarui <br><span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-blue-500">Profil Admin</span></h1>
    </div>
    ...
    <div class="grid lg:grid-cols-12 gap-12">
        <div class="lg:col-span-7">
            <div class="bg-white rounded-[40px] p-10 lg:p-14 shadow-2xl shadow-blue-100/50 border border-slate-50 relative overflow-hidden">
                <form action="{{ route('admin.admin.update', $admin->id) }}" method="POST" class="space-y-10 relative z-10">
                    @csrf
                    @method('PATCH')
                    
                    <div class="space-y-8">
                        <div class="flex items-center gap-6 p-6 bg-blue-50/50 rounded-[32px] border border-blue-100">
                            <div class="w-20 h-20 rounded-[28px] bg-gradient-to-br from-slate-900 to-slate-700 flex items-center justify-center text-white font-black text-3xl shadow-xl shadow-slate-100">
                                {{ substr($admin->name, 0, 1) }}
                            </div>
                            <div>
                                <h3 class="text-xl font-bold text-slate-800">{{ $admin->name }}</h3>
                    ...
                    <div class="space-y-6">
                        <div class="flex gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 text-sm font-bold mt-1">{{ $admin->created_at->format('d F Y') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
