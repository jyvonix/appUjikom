<x-admin-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">Administrator System</h1>
                <p class="mt-2 text-sm font-bold text-slate-400 uppercase tracking-widest">Database Admin Terdaftar</p>
            </div>
            <a href="{{ route('admin.admin.create') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-6 py-4 text-sm font-black text-white shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all sm:w-auto w-full uppercase tracking-widest">
        ...
        @if(session('error'))
        <div class="p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-600 text-xs font-bold uppercase tracking-widest animate-in fade-in slide-in-from-top-2">
            {{ session('error') }}
        </div>
        @endif
        ...
        <div class="relative">
            ...
            <form action="{{ route('admin.admin.index') }}" method="GET">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari admin..." 
                    class="block w-full rounded-[2rem] border-0 py-6 pl-16 pr-6 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-100 placeholder:text-slate-400 focus:ring-4 focus:ring-blue-500/10 transition-all font-bold">
            </form>
        </div>
        ...
        <div class="overflow-hidden bg-white shadow-sm ring-1 ring-slate-100 rounded-[2.5rem]">
            <table class="min-w-full divide-y divide-slate-100">
                <thead class="bg-slate-50/50 hidden sm:table-header-group">
                    <tr>
                        <th class="px-10 py-6 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Nama Admin</th>
                ...
                <body class="divide-y divide-slate-100">
                    ...
                    <tr class="group hover:bg-slate-50 transition-colors flex flex-col sm:table-row p-6 sm:p-0">
                        <td class="px-0 sm:px-10 py-4 sm:py-6 sm:table-cell">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 flex-shrink-0 rounded-xl bg-slate-900 text-white flex items-center justify-center font-black">
                                    {{ substr($admin->name, 0, 1) }}
                                </div>
                                <div class="font-black text-slate-900 truncate">
                                    {{ $admin->name }}
                                    @if(auth()->id() === $admin->id)
                                    <span class="ml-2 px-2 py-0.5 bg-blue-100 text-blue-600 text-[8px] uppercase tracking-tighter">Anda</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-0 sm:px-10 py-2 sm:py-6 sm:table-cell">
                            <div class="text-sm font-bold text-slate-500 truncate">{{ $admin->email }}</div>
                        </td>
                        <td class="px-0 sm:px-10 py-4 sm:py-6 sm:table-cell sm:text-right">
                            <div class="flex items-center sm:justify-end gap-3">
                                <a href="{{ route('admin.admin.edit', $admin->id) }}" class="rounded-xl bg-slate-100 p-3 text-slate-500 hover:bg-blue-50 hover:text-blue-600 transition-all">
                                ...
                                @if(auth()->id() !== $admin->id)
                                <form action="{{ route('admin.admin.destroy', $admin->id) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="3" class="px-10 py-20 text-center">
                            <p class="text-sm font-black text-slate-400 uppercase tracking-widest">Tidak ada data administrator</p>
                        </td>
                    </tr>
                    @endforelse
                </body>
