<x-admin-layout>
    <div class="relative min-h-screen pb-20 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 pt-10 space-y-10">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-white/60 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white shadow-2xl shadow-blue-900/5">
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        <span class="px-3 py-1 bg-blue-600 text-white text-[10px] font-bold uppercase tracking-widest rounded-lg shadow-lg shadow-blue-200">System Control</span>
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Administrator Management</span>
                    </div>
                    <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
                        Data <span class="text-blue-600">Administrator</span>
                    </h1>
                    <p class="text-slate-500 font-medium text-sm max-w-xl">
                        Kelola otoritas personel dengan akses kontrol penuh pada sistem SmartExam.
                    </p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                    ...
                    <div class="flex items-center gap-4 px-6 py-3 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
                        ...
                        <div class="flex flex-col">
                            <span class="text-[18px] font-bold text-slate-800 leading-none">{{ $admins->total() }}</span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Total Admin</span>
                        </div>
                    </div>

                    {{-- Add Button Refined --}}
                    <a href="{{ route('admin.admin.create') }}" class="inline-flex items-center justify-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-xs uppercase tracking-[0.1em] hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 active:scale-95 transform">
            ...
            {{-- Refined Search Bar --}}
            <div class="max-w-2xl">
                <form action="{{ route('admin.admin.index') }}" method="GET" class="relative group">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="group relative bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-blue-200/50 transition-all duration-500 flex flex-col">
                    ...
                    <div class="text-center space-y-1 mb-8">
                        <h3 class="text-lg font-bold text-slate-900 tracking-tight leading-tight group-hover:text-blue-600 transition-colors">{{ $admin->name }}</h3>
                        <p class="text-xs font-medium text-slate-400 italic">{{ $admin->email }}</p>
                    </div>
                    ...
                    <div class="mt-auto space-y-5">
                        <div class="flex items-center justify-center gap-4 py-3 bg-slate-50/50 rounded-2xl border border-slate-100/50">
                            <div class="text-center">
                                ...
                                <p class="text-[10px] font-bold text-slate-700 uppercase tracking-tight">Admin</p>
                            </div>
                            <div class="w-px h-6 bg-slate-200"></div>
                            <div class="text-center">
                                <p class="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Access</p>
                                <p class="text-[10px] font-bold text-blue-600 uppercase tracking-tight">Full</p>
                            </div>
                        </div>

                        <div class="flex items-center gap-3">
                            <a href="{{ route('admin.admin.edit', $admin->id) }}" class="flex-1 inline-flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                Edit
                            </a>
                            @if(auth()->id() !== $admin->id)
                            <form action="{{ route('admin.admin.destroy', $admin->id) }}" method="POST" onsubmit="return confirm('Hapus akun administrator ini?');" class="flex-1">
                                @csrf @method('DELETE')
                            ...
                            @else
                            <div class="flex-1 inline-flex items-center justify-center gap-2 py-3 bg-blue-50 text-blue-600 rounded-xl font-bold text-[10px] uppercase tracking-widest border border-blue-100">
                ...
                <div class="col-span-full py-32 bg-white rounded-[3rem] border border-dashed border-slate-200 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p class="text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">Node Personel Kosong</p>
                </div>
                @endforelse
            </div>
