<x-admin-layout>
    <div class="max-w-3xl mx-auto">
        <div class="mb-10 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit <span class="text-blue-600">Modul Ujian</span></h2>
                <p class="text-slate-500 font-medium text-sm">Perbarui informasi pengampu atau durasi modul ujian.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm relative overflow-hidden">
            ...
            <form action="{{ route('admin.modul.update', $modul->id) }}" method="POST" class="relative z-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <select name="user_id" required
                        ...
                        @foreach($gurus as $guru)
                            <option value="{{ $guru->id }}" {{ old('user_id', $modul->user_id) == $guru->id ? 'selected' : '' }}>{{ $guru->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $modul->nama) }}" placeholder="Contoh: Akuntansi Keuangan" required
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="deskripsi" rows="3" placeholder="Jelaskan cakupan materi modul ini..."
                        class="block w-full rounded-2xl border border-slate-200 bg-slate-50/50 py-4.5 px-6 text-slate-700 placeholder:text-slate-300 focus:bg-white focus:border-blue-500 focus:ring-8 focus:ring-blue-500/5 transition-all font-semibold text-sm">{{ old('deskripsi', $modul->deskripsi) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="is_active" required
                            class="block w-full rounded-2xl border border-slate-200 bg-slate-50/50 py-4.5 px-6 text-slate-700 focus:bg-white focus:border-blue-500 focus:ring-8 focus:ring-blue-500/5 transition-all font-semibold text-sm">
                            <option value="1" {{ $modul->is_active ? 'selected' : '' }}>Aktifkan & Publikasikan</option>
