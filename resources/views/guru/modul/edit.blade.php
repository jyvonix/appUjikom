<x-guru-layout>
    <div class="max-w-3xl mx-auto">
        <div class="mb-10 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit <span class="text-indigo-600">Modul Ujian</span></h2>
                <p class="text-slate-500 font-medium text-sm">Perbarui informasi mata pelajaran dan durasi waktu ujian.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm relative overflow-hidden">
            ...
            <form action="{{ route('guru.modul.update', $modul->id) }}" method="POST" class="relative z-10 space-y-8">
                @csrf
                @method('PUT')

                <div class="space-y-2">
                    <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama Mata Pelajaran / Modul</label>
                    <input type="text" name="nama" value="{{ old('nama', $modul->nama) }}" placeholder="Contoh: Pemrograman Dasar" required
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="deskripsi" rows="3" placeholder="Jelaskan isi modul ini..."
                        class="block w-full rounded-2xl border border-slate-200 bg-slate-50/50 py-4.5 px-6 text-slate-700 placeholder:text-slate-300 focus:bg-white focus:border-indigo-500 focus:ring-8 focus:ring-indigo-500/5 transition-all font-semibold text-sm">{{ old('deskripsi', $modul->deskripsi) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <select name="jurusan" required
                            ...
                            <option value="RPL" {{ old('jurusan', $modul->jurusan) == 'RPL' ? 'selected' : '' }}>Rekayasa Perangkat Lunak (RPL)</option>
                            <option value="MPLB" {{ old('jurusan', $modul->jurusan) == 'MPLB' ? 'selected' : '' }}>Manajemen Perkantoran & Layanan Bisnis (MPLB)</option>
                        </select>
