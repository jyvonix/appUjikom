...
<body class="antialiased" x-data="{ sidebarOpen: false }">
    ...
    <script>
        const Toast = Swal.mixin({
            ...
            customClass: {
                popup: 'rounded-3xl border border-white shadow-2xl backdrop-blur-md',
        ...
        @if(session('success'))
            Toast.fire({ icon: 'success', title: 'Berhasil', text: '{{ session('success') }}', background: '#f0fdf4', color: '#166534' });
        @endif

        @if(session('error'))
            Toast.fire({ icon: 'error', title: 'Gagal', text: '{{ session('error') }}', background: '#fef2f2', color: '#991b1b' });
        @endif
    </script>
